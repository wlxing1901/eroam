...
/// math utils
namespace eroam::math {

    // some constants
    constexpr double kDEG2RAD = M_PI / 180.0;  // deg->rad
    constexpr double kRAD2DEG = 180.0 / M_PI;  // rad -> deg
    constexpr double G_m_s2 = 9.81;            // gravity acceleration value

    // invalid id
    constexpr size_t kINVALID_ID = std::numeric_limits<size_t>::max();

    /**
     * Calculate the mean and diagonal covariance of the data in a container
     * @tparam C    container type
     * @tparam D    result type
     * @tparam Getter   get data function, receive a container data type, return a D type
     */
    ...
    /**
     * Calculate the mean and matrix form covariance of the data in a container
     * @tparam C  container type
     * @tparam int  data dimension
     * @tparam Getter  get data function, receive a container data type, return a Eigen::Matrix<double, dim,1> vector type
     */
    ...
    /**
     * @brief Merge Gaussian distributions.
     * @tparam S    scalar type
     * @tparam D    dimension
     * @param hist_m        Number of historical points
     * @param curr_n        Number of current points
     ...
     * @param new_mean      Resulting new mean
     * @param new_var       Resulting new variance
     */
    ...
    inline float GetPixelValue(const cv::Mat &img, float x, float y) {
        ...
        float yy = y - floor(y);
        return float(
                (1 - xx) * (1 - yy) * data[0] + xx * (1 - yy) * data[1] + (1 - xx) * yy * data[img.step / sizeof(T)] +
    ...
    /// Convert PCL point to Eigen vector
    ...
    /// xt: Verified that this transformation is consistent with the result of tf::createQuaternionFromRPY in ROS
    template<typename T>
    ...
    /**
     * @brief Limit a number to a given range.
     * @tparam T
     * @param num The number to limit.
     * @param min_limit The minimum value of the range.
     * @param max_limit The maximum value of the range.
     */
    ...
    /**
     * @brief Estimate a plane: solve the plane equation using colPivHouseholderQr, with a dynamic matrix to store point cloud coordinates.
     */
    template<typename T>
    inline bool
    esti_plane_dynamic(Eigen::Matrix<T, 4, 1> &abcd, const std::vector<Vec3d> &point, const double &threshold) {
        if (point.size() < 3) {
            LOG(ERROR) << "the number of points should not be less than 3, given " << point.size();
            return false;
        }
        // Marked by xiaotao:
        // Do not change the types of matrices A and b (including double and Eigen::Dynamic), issues may arise when points.size() is 3 or 4, reason unknown.
    ...
    /**
     * @brief Incremental update for a Gaussian distribution.
     ...
     * @param new_mean   Resulting merged mean
     * @param new_var2   Resulting merged variance
     */
    ...
    /**
     * @brief Pose interpolation algorithm.
     * @tparam T    Data type
     * @tparam C    Data container type
     * @tparam FT   Function to get timestamp
     * @tparam FP   Function to get pose
     * @param query_time The time to query for
     * @param data  The data container
     * @param take_pose_func Predicate to extract a pose from the data, accepts data and returns an SE3
     * @param result The query result
     * @param best_match_iter The closest match found
     *
     * NOTE: Requires query_time to be between the min and max time of the data (allows for 0.5s tolerance).
     * The map in data is assumed to be sorted by time.
    ...
    inline bool PoseInterp(double query_time, C &&data, FT &&take_time_func, FP &&take_pose_func, SE3 &result,
        ...
        double s = (query_time - take_time_func(*match_iter)) / dt;  // s=0 is the first frame, s=1 is the next
        // A bug where dt is 0 has appeared
        if (fabs(dt) < 1e-6) {
    ...
    /**
     * @brief This seems to be the Jacobian of SO3, but its exact purpose is not yet determined.
     * @param v
     * @return
     */
     ...
     * @param query_time The time to query for
     * @param data  The data
     * @param take_pose_func Predicate to extract a pose from the data
     * @param result The query result
     * @param best_match_iter The closest match found
     *
     * NOTE: Requires query_time to be between the min and max time of the data, no extrapolation.
     * The map in data is assumed to be sorted by time.
     * @return Whether interpolation was successful.
     */
    template<typename T>
    bool
    PoseInterp(double query_time, const std::map<double, T> &data, const std::function<SE3(const T &)> &take_pose_func,
        ...
        double s = (query_time - match_iter->first) / dt;  // s=0 is the first frame, s=1 is the next
    ...
    /**
     * Round a value to the nearest integer, for Eigen vector
     * @tparam S
     * @tparam n
     * @param value
     * @return
    ...
    inline Eigen::Matrix<int, n, 1> RoundToInt(const Eigen::Matrix<S, n, 1> &value) {
