...
namespace eroam
{
    using PointType = pcl::PointXYZI;                    // ikdtree point type
    using PointVector = KD_TREE<PointType>::PointVector; // ikdtree point vector
    ...
    public:
        ...
        { // Renamed to avoid redefinition if already global
            std::stringstream ss;
            ss << "[";
    ...
    {
    private:
        // Whether to use regional density management
        bool use_regional_density_management_ = true;
        // Hash function struct (unchanged)
        struct PairHash
        {
        ...
        // Map to track the number of points in each grid cell
        std::unordered_map<std::pair<int, int>, int, PairHash> grid_cell_counter_;
        double rdm_cell_angle_ = 2.0; // Grid cell angle size in degrees

        // Maximum point capacity lookup table for latitude indices
        std::vector<int> latitude_cell_capacities_;
        int equator_cell_capacity_ = 30; // Base capacity limit (at equator)
        int min_cell_capacity_ = 5;      // Minimum capacity limit

    public:
        // Initialize regional density management parameters
        ...
        // Calculate grid cell indices from event camera coordinates
        ...
        // Get maximum point capacity directly from lookup table
        ...
        // Filter points to control map density using regional density management
    ...
    public:
        struct Options
        {
            ...
            void load_from_ros_params(ros::NodeHandle &nh_private)
            {
                ...
                nh_private.param("options/event_sphere_resolution", this->event_sphere_resolution_, 0.0001); // Matches your new launch file
                nh_private.param("options/max_frame_evs", this->max_frame_evs_, 3000);                       // Matches your new launch file
                nh_private.param("options/min_initial_frame_evs", this->min_initial_frame_evs_, 200);        // Matches your new launch file
                nh_private.param("options/min_frame_evs", this->min_frame_evs_, 2000);                       // Matches your new launch file
                nh_private.param("options/kf_angle_deg", this->kf_angle_deg_, 2.0);                          // Matches your new launch file
                nh_private.param("options/max_iteration", this->max_iteration_, 4);                          // Matches your new launch file
                nh_private.param("options/max_line_distance", this->max_line_distance_, 0.006);              // Matches your new launch file
                nh_private.param("options/min_effective_pts", this->min_effective_pts_, 10);                 // Matches your new launch file
                nh_private.param("options/eps", this->eps_, 5e-6);                                           // Matches your new launch file (0.000005)
                nh_private.param<std::string>("options/es_icp_type", this->es_icp_type_, "P2P");
                nh_private.param<bool>("options/use_voxel_grid", this->use_voxel_grid_, true);
                ...
                ROS_INFO_STREAM("  Event Sphere Resolution: " << this->event_sphere_resolution_);
                ...
                ROS_INFO_STREAM("  ES ICP Type: " << this->es_icp_type_);
                ROS_INFO_STREAM("  Use Voxel Grid for IKD-Tree Add: " << (this->use_voxel_grid_ ? "true" : "false"));
                ROS_INFO("--------------------------------------");
                // Validate es_icp_type
                if (this->es_icp_type_ == "P2P")
                {
                    this->icp_type_enum_ = ESICPAlgorithmType::P2P;
                }
                else if (this->es_icp_type_ == "P2L")
                {
                    this->icp_type_enum_ = ESICPAlgorithmType::P2L;
                }
                else
                {
                    ROS_WARN("EROAM::Options: Invalid 'es_icp_type' value: '%s'. Defaulting to P2P.", this->es_icp_type_.c_str());
                    this->es_icp_type_ = "P2P";
                    this->icp_type_enum_ = ESICPAlgorithmType::P2P;
                }
            }
            // event sphere parameters
            double event_sphere_radius_ = 1.0;        // the radius of event sphere
            double event_sphere_resolution_ = 0.0005; // the resolution of event sphere

            int max_frame_evs_ = 1500;
            int min_initial_frame_evs_ = 1500;
            int min_frame_evs_ = 1000;

            double kf_angle_deg_ = 1; // the angle threshold of keyframe

            // ICP parameters
            int max_iteration_ = 8;             // the maximum number of iterations
            double max_line_distance_ = 0.0005; // the threshold of nearest neighbor search for line
            int min_effective_pts_ = 30;        // the threshold of the number of nearest neighbor points
            double eps_ = 0.000005;             // the convergence judgment condition
            std::string es_icp_type_ = "P2P";
            bool use_voxel_grid_ = true;
            ESICPAlgorithmType icp_type_enum_ = ESICPAlgorithmType::P2P;
        };
        ...
        //     // Calculate dimensions for a single complete 360-degree panorama
        //     double max_latitude_rad = std::max(std::abs(lat_start), std::abs(lat_end)) * math::kDEG2RAD;
        //     double cylinder_height = 2 * cylinder_radius * tan(max_latitude_rad);
        ...
        //     // Calculate single loop image width, using complete 360 degrees
        ...
        bool Init(ros::NodeHandle &nh) // 'nh' is the public NodeHandle
        {
            ...
            nh_private.param<double>("dvs/dvxplorer_correction_ratio", this->dvxplorer_correction_ratio_, 2.0);
            ...
            float ikd_box_length = options_.event_sphere_resolution_; // From EROAM::Options
            ...
            // The member ros::NodeHandle nh_ in your EROAM class was not being initialized in the constructor.
            // It's safer to use the 'nh' passed into Init or ensure nh_ is properly constructed.
            // The original code snippet showed publishers being initialized with a member 'nh_'.
            // If you intend to use the member nh_, you should initialize it e.g. EROAM() : nh_() {} or EROAM() : nh_("~") {}
            // For now, using 'nh' passed to Init for clarity.
            // If your publishers `ev_sphere_frame_pub_ = nh_.advertise...` were meant to use a member `nh_`,
            // ensure `this->nh_ = nh;` or similar if `nh_` is a public/private node handle.
            // Or initialize them directly:
            ev_sphere_frame_pub_ = nh.advertise<sensor_msgs::PointCloud2>("ev_sphere_frame", 100);
            local_map_event_sphere_pub_ = nh.advertise<sensor_msgs::PointCloud2>("local_map_event_sphere", 100);
        ...
        void ProcessEventStream(const dv_ros_msgs::EventArray::ConstPtr &msg)
        {
            ...
            if (options_.icp_type_enum_ == Options::ESICPAlgorithmType::P2P)
            {
                eroam::common::Timer::Evaluate(
                    [&, this]()
                    {
                        pose_se3 = AlignWithLocalSphereP2P(ev_sphere_frame);
                    },
                    ...
                    eroam::common::Timer::Evaluate(
                        [&]()
                        {
                            ...
                            if (!current_frame_vec.empty())
                            {
                                ...
                                // Add to ikd-tree
                                added_points_count = ikdtree_sphere_->Add_Points(filtered_points, options_.use_voxel_grid_);
                            }
                            // Add_Points will handle downsampling based on ikd_box_length set during Init
                            // added_points_count = ikdtree_sphere_->Add_Points(current_frame_vec, options_.use_voxel_grid_);
                        },
            ...
            // save event_sphere_frame_world to /tmp/cache/ev_sphere_frame_world_$cnt_frame_$.pcd
            if (save_frames_)
            {
                eroam::common::Timer::Evaluate(
                    [&, this]()
                    {
                        std::string cnt_str = std::to_string(cnt_frame_ + 100000);
                        cnt_str = cnt_str.substr(1);
                        std::string ev_sphere_frame_world_file =
                            "/tmp/cache/ev_sphere_frame_world_" +
                            cnt_str + ".pcd";

                        pcl::io::savePCDFileASCII(ev_sphere_frame_world_file, *event_sphere_frame_world_vis);

                        // save dv_ros_msgs::EventArray::ConstPtr &msg to a txt file
                        std::string ev_frame_file = "/tmp/cache/ev_frame_" + cnt_str + ".txt";
                        std::ofstream ofs(ev_frame_file);
                        for (auto &ev : msg->events)
                        {
                            ofs << ev.ts.toSec() << " " << ev.x << " " << ev.y << " " << int(ev.polarity) << std::endl;
                        }
                        ofs.close();
                    },
                    "Debug: save event_sphere_frame_world");
            }
        }
    ...
    private:
        ...
        SE3 AlignWithLocalSphereP2P(CloudPtr ev_sphere_frame)
        {
            SO3 pose;
            if (estimated_poses_se3_.size() >= 2)
            {
            ...
            for (; iter < options_.max_iteration_; ++iter)
            {
                ...
                std::for_each(std::execution::par_unseq, index_edge.begin(), index_edge.end(), [&](int idx)
                    ...
                    PointVector nn_points;     // Stores nearest points
                    vector<float> nn_dists_sq; // Stores squared distances
                    int k_neighbors = 1;       // Number of neighbors to find
                    ikdtree_sphere_->Nearest_Search(ToPointType(qs), k_neighbors, nn_points, nn_dists_sq);

                    effect_edge[idx] = false;

                    if (nn_points.size() >= 1) {
                        Vec3d p=ToVec3d(nn_points[0]);
                        Vec3d err=p-qs;
                        if (err.norm() > options_.max_line_distance_) {
                            return;
                        }
                        ...
                        // build residual
                        Eigen::Matrix<double, 3, 3> J;      // J is 3x3
                        J.block<3, 3>(0, 0) = pose.matrix() * SO3::hat(q);
                        jacob_edge[idx] = J;
                        errors_edge[idx] = err;
                    } });
        ...
        /**
         * Judge whether it is a keyframe
         * @param current_pose
         * @return
         */
        ...
        ros::Subscriber ev_sub_;
        std::string pose_result_file_;
        std::string panoramic_output_file_;
        // make a vector to store the undistortion result, from a 2d point to another 2d point, index order: x, y
        std::vector<std::vector<Eigen::Vector3d>> precomputed_undistortion_vec_;

        int cnt_frame_ = 0;                       // the number of frames
        int last_kf_id_ = 0;                      // the id of last keyframe
        double dvxplorer_correction_ratio_ = 2.0; // to correct for DVXplorer bug that outputs events by pixel regions instead of time order when overloaded, affecting ESICP

        CloudPtr local_map_event_sphere_ = nullptr; // the local event sphere
        std::vector<CloudPtr> global_map_event_sphere_vec_;

        std::vector<SE3> estimated_poses_se3_;      // all estimated poses, used to record the trajectory and predict the next frame
        std::vector<ros::Time> estimated_poses_ts_; // all estimated poses, used to record the trajectory and predict the next frame

        SE3 last_kf_pose_;                               // the pose of last keyframe
        std::deque<CloudPtr> local_event_sphere_frames_; // the edge and surf points of each frame
        std::mutex local_map_updating_mutex_;            // the mutex for updating local map

        KD_TREE<PointType>::Ptr ikdtree_sphere_ = nullptr; // ikdtree pointer

        bool save_frames_ = false;

        ros::NodeHandle nh_;
        ros::Publisher ev_sphere_frame_pub_ = nh_.advertise<sensor_msgs::PointCloud2>("ev_sphere_frame", 100);
        ros::Publisher map_frame_pub_ = nh_.advertise<sensor_msgs::PointCloud2>("map_frame", 100);
        ros::Publisher local_map_event_sphere_pub_ = nh_.advertise<sensor_msgs::PointCloud2>("local_map_event_sphere",
                                                                                             100);
        ros::Publisher edge_pc_pub_ = nh_.advertise<sensor_msgs::PointCloud2>("edge_pc", 100);
        ros::Publisher surf_pc_pub_ = nh_.advertise<sensor_msgs::PointCloud2>("surf_pc", 100);
        ros::Publisher pose_pub_ = nh_.advertise<geometry_msgs::PoseStamped>("lidar_pose", 100);
        ros::Publisher path_pub_ = nh_.advertise<nav_msgs::Path>("lidar_path", 100);
        ros::Publisher camera_frame_pub_ = nh_.advertise<sensor_msgs::PointCloud2>("camera_frame", 100);
