...
// brief Eigen types used in the project
...
// Vec2i can be used as index, define its "<" operator for building maps with it as key
namespace eroam {
    ...
    inline SE3 Mat4ToSE3(const Eigen::Matrix<S, 4, 4> &m) {
        /// Normalize R to prevent sophus check failure
        Quatd q(m.template block<3, 3>(0, 0).
                template cast<double>());
        q.normalize();
        return SE3(q, m.template block<3, 1>(0, 3).
                template cast<double>());
    }
